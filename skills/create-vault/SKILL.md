...
- Any skill in this plugin — `create-entity` / `create-job` / `create-performer` / `create-surface` / `create-workflow` / `create-constraint`, any `model-*` skill, or `speck` — needs a vault path before it can do its own work, whether that means finding an existing one or creating a new one.
