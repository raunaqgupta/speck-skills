...
For each confirmed workflow, delegate the file-writing to the [[create-workflow]] skill. That skill owns the canonical artifact shape (frontmatter `tags: [workflow]` + `system` + optional `job`/`performer` + `touches`, prose description, Steps table with Action/Transition/Screen columns), handles vault prerequisites (`workflows/` folder, `Templates/Workflow.md`, `.obsidian/templates.json`), links back from the anchor job's Tasks section, and refuses to overwrite existing files. **Do not rewrite that template inline here.**
