...
The original form of [Speck](https://github.com/raunaqgupta/speck): a Claude Code plugin that models a product into a local Obsidian vault through propose-and-confirm conversation, one layer at a time — entities, jobs, performers, surfaces, workflows, and constraints.

It remains the shipping product until Speck's MCP server can do what these skills do — after that, the skills' shapes and cross-checks live on as the MCP server's tool contracts. See the [main Speck repo](https://github.com/raunaqgupta/speck) for the product model and implementation this plugin is modeling toward.
